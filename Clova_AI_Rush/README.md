2021-5-18 ~ 2021-6-4 18일 동안 치뤘던 AI Rush 2021 해커톤에 대한 리뷰입니다.  
...
부족했던 점: 
1) PyTorch 모듈에 대한 이해(auto grad, no grad, Variable, Tensor, loss function 등) => docs를 처음부터 꼼꼼하게 살펴보며 어떠한 기능을 가지고 있는지 이해하고, 많이 쓰이는 기본적인 함수를 파악해보자.
2) Scikit-learn의 응용 => 모듈 설명서를 꼼꼼하게 읽어보자
3) 수학 논리적 탐구를 통한 모델 원리 이해(모델과 알고리즘에 대해 대략적으로, 경험적으로 설명할 수 있지만, 논리적으로 깊게 원리를 설명할 수가 없다는 점) => '밑바닥부터 배우는 딥러닝' 1권, 3권 교재를 통하여 공부
4) 모델링 경험 => 커뮤니티를 통해 논문과 Tip을 접하고 기록하자
